...
export const VideoStatus = ({ status }: IVideoStatus) => {
  switch (status) {
    case EVideoStatuses.IN_QUEUE:
      return (
        <div className="text-default-white bg-[#8e8e8e] w-fit flex gap-2 items-center pe-[10px] ps-[6px] rounded-[10px] text-xs py-[2px]">
          <span className="w-2 h-2 rounded-full bg-[#eaeaea]" />В очереди
        </div>
      );
    case EVideoStatuses.IN_PROCESS:
      return (
        <div className="text-default-white bg-[#ECB010] w-fit flex gap-2 items-center pe-[10px] ps-[6px] rounded-[10px] text-xs py-[2px]">
          <span className="w-2 h-2 rounded-full bg-[#FBFF00]" />В обработке
        </div>
      );
    case EVideoStatuses.SUCCESFULL:
      return (
        <div className="text-default-white bg-[#288442] w-fit flex gap-2 items-center pe-[10px] ps-[6px] rounded-[10px] text-xs py-[2px]">
          <span className="w-2 h-2 rounded-full bg-[#00D33D]" />
          Обработано
        </div>
      );
    case EVideoStatuses.FAIL:
      return (
    ...
    default:
      return (
        <div className="text-default-white bg-[#8e8e8e] w-fit flex gap-2 items-center pe-[10px] ps-[6px] rounded-[10px] text-xs py-[2px]">
          <span className="w-2 h-2 rounded-full bg-[#eaeaea]" />
          Нет видео
        </div>
      );
  }
